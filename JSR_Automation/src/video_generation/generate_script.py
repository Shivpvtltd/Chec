...
Generates complete YouTube video scripts in Hinglish with 10+ minute enforcement
...
- Episode: {episode}
- Title: {title}
- **CRITICAL: Target Duration MUST BE 12-15 MINUTES (MINIMUM 1800-2200 words when spoken)**
...
- Language: HINGLISH (Roman Hindi - Hindi words in English script)

**10+ MINUTE ENFORCEMENT RULES:**
1. HOOK (0-45 seconds / 100-130 words):
...
2. PROBLEM AGITATION (45-120 seconds / 200-280 words):
   - Describe the pain point in vivid detail
   - Use "tum" (you) extensively for personalization
   - Make viewer feel understood and frustrated

3. PROMISE (120-180 seconds / 150-200 words):
   - Clear statement of what they'll learn
   - "Aaj ke baad tum kabhi fooled nahi hoge"
   - Build anticipation for the reveal

4. MAIN CONTENT (9-12 minutes / 1400-1800 words) - **THIS IS CRITICAL:**
   - 4-5 distinct sections with clear transitions
   ...
   - Include "Research ke mutabik" (According to research) for credibility
   - Add "Mere saath hua tha" (Happened with me) type personal touches
   - **MUST BE DETAILED - NO RUSHING THROUGH TOPICS**

5. PRACTICAL TIPS (2-3 minutes / 350-450 words):
   - 5-7 actionable steps (NOT just 3-4)
   - Specific, not generic ("Phone uthane se pehle 2 baar socho" not just "Be careful")
   - Include early warning signs to watch for
   - Add real-life implementation examples

6. CONCLUSION (1.5-2 minutes / 250-350 words):
   - Summary of key insights (NOT just one)
   - Emotional reinforcement: "Tum smart ho, bas aware hona hai"
   - Call-to-action: Comment, share with friend, subscribe
   - Teaser for next episode
   - Personal closing message

BARK TTS MARKERS (USE EXACTLY):
[PAUSE-1] [PAUSE-2] [PAUSE-3] [EMPHASIS] [WHISPER] [EXCITED] [SERIOUS] [QUESTION]

SCENE MARKERS FOR VIDEO EDITING:
...
{{
  ...
  "script": {{
    "hook": "Text with [PAUSE-1] markers",
    "problem_agitation": "Text with markers",
    "promise": "Text with markers",
    "main_content": [
      {{
        "section_title": "Title",
        "content": "Detailed content with markers and [SCENE: type]"
      }}
    ],
    "practical_tips": [
      {{
    ...
    "conclusion": "Conclusion text with markers",
    "word_count": 2000,
...
def generate_script(category, sub_category, episode, run_id):
    ...
    try:
        ...
        if word_count < 1800:
            print(f"⚠️ Word count {word_count} is below minimum 1800")
        ...
        script_data['generation_info'] = {
            ...
            'word_count_validated': word_count >= 1800,
